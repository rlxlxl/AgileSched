...
| Команда | Действие |
|---------|----------|
| `/start` | Меню (один раз; дальше меню возвращается само) |
...
- `/schedule` — пропуск выбора отдела/сотрудника, сразу «Как заполнить?» (пошагово / одним сообщением).

Если профиля у ответственного нет — бот попросит выполнить `/my`.

Если привязан **личный** профиль (`/my`), при `/schedule` отдел и сотрудник тоже пропускаются (сразу выбор режима заполнения).
...
- `/start` — меню
- `/schedule` — заполнить: **пошагово** или **одним сообщением** (`Пн 9:00-18:00, Вт 10:00-19:00`)
- `/my` — профиль: отдел, сотрудник, **ставка**, **обед**
- `/myschedule` — своё расписание + норма
- `/status` — Excel, ставка, обед, напоминания
- `/remind on|off` — включить/выключить напоминания
- `/remind hour 9` / `/remind day 1` — час и день недели (1=пн), таймзона `Europe/Moscow`
- `/cancel` — отмена и возврат в меню

После любого действия бот снова показывает меню — повторный `/start` не нужен.
...
Часы из ячеек Excel (`0.5` / `1` / `д`), затем из каждого рабочего дня вычитается **обед** (0 / 30 / 45 / 60 / 90 мин). Норма сверяется с **нетто**-часами.

### Напоминания

По умолчанию: понедельник 9:00 МСК. В праздники РФ не шлёт. Повтор в тот же день не шлёт. Только Telegram.

### Свободный ввод

После выбора сотрудника → «Одним сообщением» → текст вида `Пн 9:00-18:00, Вт 10:00-19:00, Ср 9-18`.
